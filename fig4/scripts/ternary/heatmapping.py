...
def heatmap(data, scale, vmin=None, vmax=None, cmap=None, ax=None,
            ...
            permutation=None, use_rgba=False, cbarlabel=None, cticklabels=None, cb_kwargs=None):
    """
    ...
    if colorbar:
        colorbar_hack(ax, vmin, vmax, cmap, scientific=scientific,
                      cbarlabel=cbarlabel, cticklabels=cticklabels, **cb_kwargs)
    return ax
...
def background_color(ax, color, scale, zorder=-1000, alpha=None):
    ...
    xs, ys = unzip(vertices)
    poly = ax.fill(xs, ys, facecolor=color, edgecolor=color, zorder=zorder, alpha=0)
    return poly
